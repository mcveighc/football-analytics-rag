...
def test_match_report_displays_expected_report():
    ...
    match_summary_data = pd.DataFrame([
        {"match_id": 1, "match_date": f"{match_date}", "team": f"{away_team_name}",
            "home_or_away": "away", "goals": away_goals, "shots": away_shots, "total_xg": away_xg},
        {"match_id": 1, "match_date": f"{match_date}", "team": f"{home_team_name}",
    ...
    # assert
    assert output[0] == f"{home_team_name} vs {away_team_name} - {match_date}"
    assert output[1] == f"Score: {home_goals:.0f}-{away_goals:.0f}"
    assert output[2] == f"Shots: {home_shots:.0f}-{away_shots:.0f}"
    assert output[3] == f"Expected goals: 1.65-1.92"
